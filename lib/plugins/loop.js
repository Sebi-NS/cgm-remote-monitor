...
Search for "RemoteBolus"

carePortal.js
	refreshEventTypes()
		Creates an inputMatrix from event types with true/false visibility values for each field
		Maps event to the submit hook used when user submits data.
	This part: $('#remoteBolusLabel').css('display', displayType(inputMatrix[eventType]['remoteBolus']));
		Sets the css 'display' to 'none' to hide label, depending on inputMatrix
			Sets display to '' if it _should_ display it.
		I can toggle on the new fields here.
	This part: resetIfHidden(inputMatrix[eventType]['remoteBolus'], '#remoteBolus');
		Seems to reset the fields if they are hidden.
	This part: $('#remoteBolus').val('');
		Sets defaults on fields, after refreshEventTypes called
		This may be where I add the IOB and COB
			the client seems to be sourced from index.js
			cob.js is the plugin for calculating
			Other search terms: prepareBGs addExtraData loopalyzer cobTotal
	This part:
		part of gatherData()
		Where the form values are pulled for confirmation dialog and the submit hook.
		I probably need to add IOB/COB here.
	This part: pushIf(data.remoteBolus, translate('Remote Bolus') + ': ' + data.remoteBolus);
		Where the data from gatherData() is put in a confirmation dialogue
		I probably need to add IOB/COB here.
lib/plugins/loop.js
	Creates a dictionary of fields with booleans values, used in carePortal.refreshEventTypes
	This toggles field visibility
lib/server/loop.js
	Sends the push notification.
	I'd add the payload for iob/cob here.
index.html
	Defines the HTML
	I'd probably add labels for IOB/COB here.
Getting at COB and IOB
	sandbox.js
		Has treatment info used by plugins
		Used by COB plugin		
	careportal
		Has a plugin and also is in clients?
	loop.js
		A plugin
		Seems like the place that creates the events -- including remote bolus / carbs
		It has access to the sbx (sandbox)
		Maybe where I can fetch the carbs on board?
*/
function init (ctx) {
  ...
  loop.getEventTypes = function getEventTypes (sbx) {
    ...
    //  var cobInGrams = prop.lastLoop.cob.cob;
    //  cobInGrams = Math.round(cobInGrams);
    //}
   ...
   // TODO: add OTP entry
    ...
    return [
      ...
      },
      {
        ...
        , otp: true
        //, carbsInGrams: cobInGrams
        , submitHook: postLoopNotification
      },
      {
        //TODO: This is where I could pass more params?
