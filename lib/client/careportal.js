...
function init (client, $) {
  ...
  function refreshEventTypes() {
    ...
    _.forEach(careportal.allEventTypes, function each (event) {
      inputMatrix[event.val] = _.pick(event, ['otp','remoteCarbs', 'remoteAbsorption', 'remoteBolus', "cob", 'bg', 'insulin', 'carbs', 'protein', 'fat', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split', 'sensor', 'reasons', 'targets']);
  ...
  careportal.filterInputs = function filterInputs (event) {
    ...
    $('#cobLabel').css('display', displayType(inputMatrix[eventType]['cob']));
    ...
    resetIfHidden(inputMatrix[eventType]['cob'], '#cob');
  ...
  careportal.prepare = function prepare () {
    ...
    var cob = Math.round(client.sbx.properties.cob.cob * 10) / 10;
    $('#cob').val(cob);
    $('#cobValue').val(client.sbx.properties.cob.cob);
  ...
  function buildConfirmText (data) {
    ...
    pushIf(data.cob, translate('COB') + ': ' + data.cob);
    pushIf(data.cobValue, translate('COB Value') + ': ' + data.cobValue);
