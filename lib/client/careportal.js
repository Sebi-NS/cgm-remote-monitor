...
function init (client, $) {
  ...
  function refreshEventTypes() {
    ...
    _.forEach(careportal.allEventTypes, function each (event) {
      inputMatrix[event.val] = _.pick(event, ['otp','remoteCarbs', 'remoteAbsorption', 'remoteBolus', 'remoteCOB', 'remoteIOB', 'remoteBGNow', 'bg', 'insulin', 'carbs', 'protein', 'fat', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split', 'sensor', 'reasons', 'targets']);
  ...
  careportal.filterInputs = function filterInputs (event) {
    ...
    $('#remoteCOBLabel').css('display', displayType(inputMatrix[eventType]['remoteCOB']));
    $('#remoteIOBLabel').css('display', displayType(inputMatrix[eventType]['remoteIOB']));
    $('#remoteBGNowLabel').css('display', displayType(inputMatrix[eventType]['remoteBGNow']));
    ...
    $('#nutrition').css('display', displayType(inputMatrix[eventType]['carbs']));
    ...
    resetIfHidden(inputMatrix[eventType]['remoteCOB'], '#remoteCOB');
    resetIfHidden(inputMatrix[eventType]['remoteIOB'], '#remoteIOB');
    resetIfHidden(inputMatrix[eventType]['remoteBGNow'], '#remoteBGNow');
  ...
  careportal.prepare = function prepare () {
    ...
    //$('#cob').val(cob.cobTotal(client.sbx.data.treatments, client.sbx.data.devicestatus, client.sbx.data.profile, record.eventTime, record.profile).cob, 0.01);
    //var loop = client.plugins('loop');
    //$('#cob').val(inputMatrix[eventType]['cobInGrams']);

    //Didn't work
    //var cobResult = client.plugins('cob').cobTotal(client.sbx.data.treatments, client.sbx.data.devicestatus, client.sbx.data.profile, Date.now());
    
    //Try plugin properties
    console.log('Last COB: ', client.sbx.properties.cob);
    console.log('Last IOB: ', client.sbx.properties.iob);
    console.log('BGNow: ', client.sbx.properties.bgnow);
    console.log('loop: ', client.sbx.properties.loop);

    var remoteCOB = Math.round(client.sbx.properties.cob.cob * 10) / 10;
    $('#remoteCOB').val(remoteCOB);
    $('#remoteCOBValue').val(client.sbx.properties.cob.cob);

    var remoteIOB = Math.round(client.sbx.properties.iob.iob * 10) / 10;
    $('#remoteIOB').val(remoteIOB);
    $('#remoteIOBValue').val(client.sbx.properties.iob.iob);

    var remoteBGNow = Math.round(client.sbx.properties.bgnow.last * 10) / 10;
    $('#remoteBGNow').val(remoteBGNow);
    $('#remoteBGNowValue').val(client.sbx.properties.bgnow.last);
  ...
  function gatherData () {
    ...
    var data = {
      ...
      , remoteCOB: $('#remoteCOB').val()
      , remoteIOB: $('#remoteIOB').val()
      , remoteBGNow: $('#remoteBGNow').val()
  ...
  function buildConfirmText (data) {
    ...
    pushIf(data.remoteCOB, translate('COB') + ': ' + data.remoteCOB);
    pushIf(data.remoteIOB, translate('IOB') + ': ' + data.remoteIOB);
    pushIf(data.remoteBGNow, translate('BG') + ': ' + data.remoteBGNow);
